...
clean:
	...
	rm -f *.f90
	rm -f *.mod
	cd gc; FC=$(SFC) make HPC=y realclean
	cd gc/GCHP; make USE_EXTERNAL_GRID=y MET=geos-fp the_nuclear_option
	cd gc/GCHP; make clean
	rm gc/GCHP/*.o
...
chemics_init.o: compile_chem module_input_chem_data.o module_tropopause.o module_upper_bc_driver.o wrfgc_convert_state_mod.o
	$(RM) $@
...
chem_driver.o: compile_chem ../dyn_em/module_convtrans_prep.o module_input_chem_data.o module_chem_utilities.o module_tropopause.o  module_upper_bc_driver.o wrfgc_convert_state_mod.o
	$(RM) $@
	sed -e "s/grid%mu/gridmu/g" -e "s/grid%Mu/gridMu/g" -e "s/^\!.*'.*//" -e "s/^ *\!.*'.*//" $*.F > $*.G
	$(CPP) -I$(WRF_SRC_ROOT_DIR)/inc $(CPPFLAGS) $(OMPCPP) $*.G  > $*.H
	sed -e "s/gridmu/grid%mu/g" -e "s/gridMu/grid%Mu/g" $*.H > $*.bb
